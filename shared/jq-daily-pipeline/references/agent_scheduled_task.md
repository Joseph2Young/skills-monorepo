...
跑 7.5 年（2019-01-01 ~ 昨日）回测，Sharpe ≥ 1.0 通过后按聚宽策略审查分类提示词 v1.0
分类（12 大类），按 {年份}_{类型}_{简称}_{标题核心}.md 命名后上传到 IMA 知识库。
...
2. 跑 run_daily.sh
3. 汇报：扫描了多久 / Top 3 / 几个通过查重 / 几个跑回测 / 几个 Sharpe ≥ 1.0 / 几个最终入库
4. 失败的策略说明原因
```
